...
export async function buildApp(app: FastifyInstance): Promise<FastifyInstance> {
  ...
  await app.register(fp(async (a) => {
    const kafka = new Kafka({ clientId: 'gradebook-service', brokers: (process.env['KAFKA_BROKERS'] ?? 'kafka:9092').split(',') })
  ...
  app.get('/health', { logLevel: 'silent' }, async (_req, reply) => reply.send({ status: 'healthy', service: 'gradebook-service', timestamp: new Date().toISOString() }))
