...
import { startNotificationProducer } from './services/notification-producer.js'
...
export async function buildApp(app: FastifyInstance): Promise<FastifyInstance> {
  ...
  // ── Kafka Producer + Notification Consumer ─────────────
  await app.register(fp(async (a) => {
    const kafka = new Kafka({ clientId: 'attendance-service', brokers: (process.env['KAFKA_BROKERS'] ?? 'kafka:9092').split(',') })
    const producer = kafka.producer()
    await producer.connect()
    a.decorate('kafkaProducer', producer)

    const notifConsumer = await startNotificationProducer(a.db, producer)
    a.addHook('onClose', async () => { await producer.disconnect(); await notifConsumer.disconnect() })
  }))
  ...
  app.get('/health', { logLevel: 'silent' }, async (_req, reply) => {
    reply.send({ status: 'healthy', service: 'attendance-service', timestamp: new Date().toISOString() })
  })
  ...
  await app.register(qrcodeRoutes,     { prefix: '/api/v1' })
